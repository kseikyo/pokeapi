...
export const pokemonHandler = rest.get(
  "https://pokeapi.co/api/v2/pokemon/pikachu",
  (req, res, ctx) => {
    return res(ctx.delay(1500), ctx.json(MOCK_POKEMON));
  }
);

export const pokemonHandlerException = rest.get(
  "https://pokeapi.co/api/v2/pokemon/this-should-not-find-anything",
  (req, res, ctx) =>
    res(ctx.delay(1500), ctx.status(404), ctx.text(MOCK_MESSAGE_ERROR))
);
